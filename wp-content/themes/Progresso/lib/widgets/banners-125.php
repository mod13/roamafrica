...
class ThematerBanners125 extends WP_Widget 
{
    ...
    {
        $widget_options = array('description' => __('添加125x125像素的图片.', 'themater') );
        $control_options = array( 'width' => 600);
		$this->WP_Widget('themater_banners_125', '&raquo; 125x125图片', $widget_options,$control_options);
    }
    ...
    {	
        ...
        <div style="margin-bottom: 20px;">
            <a class="button" onclick="themater_125_banner_new();" >添加新的图片</a> &nbsp; &nbsp; <input type="checkbox" name="<?php echo $this->get_field_name('randomize'); ?>" <?php checked('true', $instance['randomize']); ?> value="true" /> 随机图片显示
        </div>
        <?php
            if(is_array($get_banners)) {
                foreach($get_banners as $banner_id=>$banner_source) {
                    ...
                    <div class="tt-clearfix" style="padding: 0 0 20px 0; border-bottom: 1px solid #ddd; margin-bottom: 20px;" id="container_<?php echo $this->get_field_id($banner_id); ?>">
                        <div style="width: 133px; float: right; ">
                            ...
                            <div><a class="button"  onclick="themater_125_banner_preview('<?php echo $this->get_field_id($banner_id); ?>');">预览</a> <a class="button tt-button-red" onclick="if (confirm('The selected banner will be deleted! Do you really want to continue?')) { themater_125_banner_delete('<?php echo $this->get_field_id($banner_id); ?>'); } return false;">删除</a></div>
                        </div>
                ...
                <div class="themater_125_banner_prototype" style="display: none;">
                    <div class="tt-clearfix" style="padding: 0 0 20px 0; border-bottom: 1px solid #ddd; margin-bottom: 20px;" id="container_the__id__">
                        <div style="width: 133px; float: right; ">
                            ...
                            <div><a class="button"  onclick="themater_125_banner_preview('the__id__');">预览</a> <a class="button tt-button-red" onclick="if (confirm('The selected banner will be deleted! Do you really want to continue?')) { themater_125_banner_delete('the__id__'); } return false;">删除</a></div>
                        </div>
                        <div style="margin-right: 150px;">
