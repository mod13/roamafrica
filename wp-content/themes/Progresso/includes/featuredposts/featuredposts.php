<?php
    ...
    class Themater_FeaturedPosts
    {
        ...
        var $defaults = array(
            ...
            'enabled_positions' => array('homepage'=>'首页', 'categories' => '分类页', 'tags' => '标签页', 'archives' => '存档页', 'single' => '边栏页面', 'pages' => '单页面', 'searches'=>'搜索结果页'),
        ...
        var $moreoptions =  array(
            ...
            'sync' => 'In/Out同时显示',
            'pause' => '暂时悬停',
            'pauseOnPagerHover' => '当鼠标悬停在链接上，显示页面',
            'continuous' => '当前一个完成后立即开始的下一个转型（将覆盖其他计时选项，如：延迟，速度等）'
        );
        ...
        function featuredposts_source()
        {
            ...
            $featuredposts_sources = array('custom'=> '自定义幻灯片', 'category'=> '分类文章', 'posts' => '选择文章ID', 'pages' => '选择面页ID');
        ...
        function featuredposts_custom_slides()
        {
            ...
            <div style="margin-bottom: 20px;">
                <a class="button" href="#new_custom_slide_a" onclick="featured_slider_new();" >添加新的幻灯片</a>
            </div>
                ...
                ?>
                    <div style="padding: 0 0 0 0; border-bottom: 1px solid #ddd; margin-bottom: 20px;" id="featured_custom_slide_<?php echo $key; ?>">
                        <div style="background: #efefef; padding: 5px; margin-bottom: 5px; font-weight: bold;">
                            幻灯片 #<?php echo $no; ?> - <?php echo $custom_slide['title']; ?>
                            &nbsp; <a class="button" href="javascript:themater_showHide('featured_custom_slide_content_<?php echo $key; ?>');">编辑</a>
                            &nbsp; <a class="button tt-button-red" href="javascript:featured_slider_delete('<?php echo $key; ?>');">删除</a>
                        </div>
                        <div class="fp-form-element" id="featured_custom_slide_content_<?php echo $key; ?>" style="display: none;">
                             <table width="100%">
                                <tr>
                                    <td class="tt-inline-label" style="width: 15%;" valign="top">图片地址:</td>
                                ...
                                <tr>
                                    <td class="tt-inline-label" style="width: 15%;" valign="top">链接地址:</td>
                                 ...
                                 <tr>
                                    <td class="tt-inline-label" style="width: 15%;" valign="top">标题:</td>
                                ...
                                <tr>
                                    <td class="tt-inline-label" style="width: 15%;" valign="top">内容:</td>
                ...
                <div id="new_custom_slide_prototype" style="display: none;">
                    ...
                    <div style="padding: 0 0 0 0; border-bottom: 1px solid #ddd; margin-bottom: 20px;" id="featured_custom_slide_the__id__">
                        <div style="background: #eee; padding: 5px; margin-bottom: 5px; font-weight: bold;">
                            <span style="color: green;">新增幻灯片</span>
                            &nbsp; <a class="button" href="javascript: themater_showHide('featured_custom_slide_content_the__id__');">编辑</a>
                            &nbsp; <a class="button tt-button-red" href="javascript: featured_slider_delete('the__id__');">删除</a>
                        </div>
        ...
        {
             $this->theme->admin_option(array('文章选项', 15), 
                '文章选项', 'featuredposts_enabled', 
                'checkboxes', $this->defaults['enabled_in'], 
                array('options' => $this->defaults['enabled_positions'], 'help'=> '启用幻灯片功能的页面：', 'display'=>'extended-top')
            );
            
        
            $image_sizes = $this->defaults['image_sizes'] ? '<br>建议图像大小为 <strong>' . $this->defaults['image_sizes'] . '</strong>' : '';
            $this->theme->admin_option('文章选项', 
                '文章标题图片', 'featuredposts_images', 
                'content','<i>这些图片来源于各页面的标题图片集中，' . $image_sizes . '</i>'
            );
            
            $this->theme->admin_option('文章选项', 
                '文章图片来源', 'featuredposts_source', 
           ...
           if(is_array($featuredposts_custom_slides_raw)) {
            ...
            $this->theme->admin_option('文章选项', 
             ...
             $this->theme->admin_option('文章选项', 
                '幻灯片效果', 'featuredposts_effect', 
                'select', $this->defaults['effect'], 
                array('options'=> $this->effects)
            );
            
             $this->theme->admin_option('文章选项', 
                '其他选项', 'featuredposts_misc_options_info', 
                'content', ''
            );
            ...
            $this->theme->admin_option('文章选项', 
                '"更多阅读" 链接文本', 'featuredposts_readmore', 
                'text', $this->defaults['readmore'], 
                array('help'=> '留空隐藏',  'display'=>'inline')
            );
            
            $this->theme->admin_option('文章选项', 
                '摘录长度', 'featuredposts_excerpt_length', 
                'text', $this->defaults['excerpt_length'], 
                array('suffix'=> '字', 'style'=>'width: 80px;', 'display'=>'inline')
            );
            
            $this->theme->admin_option('文章选项', 
                '幻灯片超时', 'featuredposts_timeout', 
                'text', $this->defaults['timeout'], 
                array('suffix'=> ' 毫秒', 'style'=>'width: 80px;', 'display'=>'inline', 'help' => '幻灯片转换之间的毫秒（0禁用自动提前）')
            );
            
            $this->theme->admin_option('文章选项', 
                '幻灯片延迟', 'featuredposts_delay', 
                'text', $this->defaults['delay'], 
                array('suffix'=> ' 毫秒', 'style'=>'width: 80px;', 'display'=>'inline', 'help'=>'额外的延迟时间（毫秒）（提示：第一次转换可以是负数）')
            );
            
            $this->theme->admin_option('文章选项', 
                '幻灯片速度', 'featuredposts_speed', 
                'text', $this->defaults['speed'], 
                array('suffix'=> ' 毫秒', 'style'=>'width: 80px;', 'display'=>'inline', 'help'=>'幻灯片转换的速度')
            );
            
            $this->theme->admin_option('文章选项', 
                '滑入速度', 'featuredposts_speedIn', 
                'text', $this->defaults['speedIn'], 
                array('suffix'=> ' 毫秒', 'style'=>'width: 80px;', 'display'=>'inline', 'help'=>'幻灯片滑入的速度 ')
            );
            
            $this->theme->admin_option('文章选项', 
                '滑出速度', 'featuredposts_speedOut', 
                'text', $this->defaults['speedOut'], 
                array('suffix'=> ' 毫秒', 'style'=>'width: 80px;', 'display'=>'inline', 'help'=>'幻灯片滑出的速度 ')
            );
        }
    }
?>