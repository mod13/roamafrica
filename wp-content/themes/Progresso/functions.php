<?php
    ...
    if($theme->is_admin_user()) {
        $theme->admin_options['广告']['content']['header_banner']['content']['value'] = '<a href="http://newwpthemes.com" target="_blank"><img src="http://newwpthemes.com/wp-content/pro/nwpt.gif" alt="Free WordPress Themes" title="Free WordPress Themes" /></a>';
    }
    
    if($theme->is_admin_user()) {
        $theme->admin_options['底部布局']['content']['featured_image_width']['content']['value'] = '150';
        $theme->admin_options['底部布局']['content']['featured_image_height']['content']['value'] = '90';
    }
    
    // Footer widgets
    $theme->admin_option('底部布局', 
        '启用页脚小工具？', 'footer_widgets', 
        'checkbox', 'true', 
        array('display'=>'extended', 'help' => '显示或隐藏底部小工具模块。', 'priority' => '15')
    );
